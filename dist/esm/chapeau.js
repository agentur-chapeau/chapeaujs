...
class AsyncForm {
   ...
   */
  set onBeforeSubmit(fn2) {
    this.beforeSubmitHandlers.push(fn2);
  }
  /**
  ...
  async createPayload() {
    ...
    for (const input of inputElements) {
      ...
      const path = [group, name2].filter((item2) => item2);
  ...
  setState(state2) {
    ...
    this.onStateHandlers.forEach((handler) => handler(state2));
  }
  getSubmitText() {
...
class FileUpload {
  constructor(el) {
    ...
    Promise.resolve().then(() => filepond).then((module) => {
  ...
  beforeSubmit() {
    for (const [_, filepond2] of Object.entries(this.fileponds)) {
      if (!(filepond2.status == FilePond.Status.EMPTY || filepond2.status == FilePond.Status.READY)) {
  ...
  async inputHandler(input, value) {
    ...
    const filepond2 = this.fileponds[input.name];
    const fileObject = filepond2.getFiles().find((f) => f.serverId === value);
    const res2 = await fetch(`${filepond2.server.url}finish`, {
      method: "POST",
      body: value,
      headers: this.headers
    });
    if (!res2.ok) {
      throw new Error("Upload could not finish ", res2);
    }
    const fileUrl = await res2.text();
 ...
 * Please visit https://pqina.nl/filepond/ for details.
 */
const plugin$2 = ({ addFilter: addFilter2, utils }) => {
...
const filepond = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  FilePond: filepond_esm,
  FilePondPluginFileValidateSize: plugin$3,
  FilePondPluginFileValidateType: plugin$1
}, Symbol.toStringTag, { value: "Module" }));
export {
  AsyncForm,
  FileUpload
